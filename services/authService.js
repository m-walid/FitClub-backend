...
const verifyAccount = async (verifyBody) => {
  ...
  if (account.isVerified) throw new Exception("Account already verified, please login");
  if (account.otp.code !== verifyBody.code || !checkOtpExpValid(account.otp.updatedAt)) throw new Exception("Invalid OTP");
  const updatedAccount = await accountRepository.updateAccountById(account.id, { isVerified: true });
  const tokenPayload = {
    id: account.id,
    role: account.role,
  };
